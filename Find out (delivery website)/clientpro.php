...
try{ 
    ...
    if(isset($_POST['submit'])){
        ...
        $stm1="UPDATE clientt set username='$nom' where id=$id";
        $stm=$ddb->prepare($stm1);
        $stm -> execute();
        $stm2="UPDATE clientt set email='$email' where id=$id";
        $stm=$ddb->prepare($stm2);
        $stm -> execute();
        $stm3="UPDATE clientt set number='$phone'  where id=$id";
        $stm=$ddb->prepare($stm3);
        $stm -> execute();
        if(!empty($image_name)){
          $stm5="UPDATE clientt set image='$image_name' where id=$id";
          $stm=$ddb->prepare($stm5);
          $stm -> execute();
      }else{
          /*just let  the fucking image as you find idiot*/
      }
         header('location:clientpro.php');
...
<a href="clientsearch.php" class="home"><svg class="a"  xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512"><path fill="#208cdf" d="M575.8 255.5c0 18-15 32.1-32 32.1h-32l.7 160.2c0 2.7-.2 5.4-.5 8.1V472c0 22.1-17.9 40-40 40H456c-1.1 0-2.2 0-3.3-.1c-1.4 .1-2.8 .1-4.2 .1H416 392c-22.1 0-40-17.9-40-40V448 384c0-17.7-14.3-32-32-32H256c-17.7 0-32 14.3-32 32v64 24c0 22.1-17.9 40-40 40H160 128.1c-1.5 0-3-.1-4.5-.2c-1.2 .1-2.4 .2-3.6 .2H104c-22.1 0-40-17.9-40-40V360c0-.9 0-1.9 .1-2.8V287.6H32c-18 0-32-14-32-32.1c0-9 3-17 10-24L266.4 8c7-7 15-8 22-8s15 2 21 7L564.8 231.5c8 7 12 15 11 24z"/></svg></a>
  ...
  <div class="main">
                     <div class="ban">
                        <img src="<?="img/".$client['banner']?>">
                        <i class="fas fa-upload" id="pen" onclick="document.getElementById('in').click()" ></i>
                                <input type="hidden" name="MAX_FILE_SIZE" value="100000">
                                <input type="file" name="imgban" id="in" style="display: none;" onchange="reload()">
                     </div>
                     ...
                     <div class="logg">
                        ...
                        <i class="fas fa-pen" id="pen2" onclick="document.getElementById('in').click()" ></i>
                                <input type="hidden" name="MAX_FILE_SIZE" value="100000">
                                <input type="file" name="image" id="in" style="display: none;" onchange="reload()">
                     </div>
                        <div class="ne">
                                    <p class="p1"><?=$client['username']?></p>
                                    <p class="p2" ><?=$client['email']?></p>
                        </div>
                        <div class="part">
                            <div class="part1"><a href="clientpro.php"><i class="fa-solid fa-user"></i><h2 >Account Information</h2></a></div>
                            <div class="mid">  </div>
                            <div class="part2"><a href="clientsett.php"><i class="fa-solid fa-cog"></i><h2 >Account settings</h2></a></div>
                        </div>
                        
                        <form method="post">
                            
                            <div class="ne1" style="width: 1000px;">
                               <div class="lab"><label class="labin"><em>Name</em><em class="p2"><?=$client['username']?></em></label><label class="btn" id="a1" onclick="show(1,'a1')" ondblclick="hide(1)">Change</label> 
                                <div class="hidden" id="1"><input type="text" name="nom" value="<?=$client['username']?>" class="input1"  ><input type="submit" class="btn1" value="Update" name="submit"></div>  </div> 

                               <div class="lab"><label class="labin"><em>Email</em><em class="p2"><?=$client['email']?></em></label><label class="btn" id="a2" onclick="show(2,'a2')">Change</label>
                               <div class="hidden" id="2"><input type="text" name="email" value="<?=$client['email']?>" class="input1" ><input type="submit" class="btn1" value="Update" name="submit"></div></div> 
                                
                               <div class="lab"><label class="labin"><em>Phone</em><em class="p2"><?=$client['number']?></em></label><label class="btn" id="a3" onclick="show(3,'a3')">Change</label>
                                        <div class="hidden" id="3"><input type="text" name="phone" value="<?=$client['number']?>" class="input1" ><input type="submit" class="btn1" value="Update" name="submit"></div></div> 
